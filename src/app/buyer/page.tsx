...
import { ShoppingCart, CheckCircle2, RotateCcw, ArrowRight, ShieldCheck } from "lucide-react";
...
export default function BuyerPage() {
  ...
  const sellerRef = useMemo(() => (activeStoreId ? doc(db, "sellers", activeStoreId) : null), [db, activeStoreId]);
  const { data: seller } = useDoc<Seller>(sellerRef);

  const [currentTxnId, setCurrentTxnId] = useState<string | null>(null);
  const txnRef = useMemo(() => (currentTxnId ? doc(db, "transactions", currentTxnId) : null), [db, currentTxnId]);
  const { data: txn } = useDoc<Transaction>(txnRef);
  ...
  const handleSearch = () => {
    ...
    setActiveStoreId(storeIdInput);
    setStep("STOREFRONT");
  };

  const handleRequestQuote = async () => {
    if (!selectedProduct || !activeStoreId || !db) return;
    setLoading(true);

    const newTxn = {
      txn_id: "", // Temporary, updated after doc creation
      status: "PENDING" as const,
      buyer_name: buyerForm.buyer_name || "Anonymous",
    ...
    await updateDoc(docRef, { txn_id: docRef.id });
  ...
  const markAsPaid = async () => {
    if (!txnRef) return;
    await updateDoc(txnRef, { status: "PAYMENT_SUBMITTED" });
    setStep("PENDING");
  };

  if (txn?.status === "AWAITING" && step === "PENDING") setStep("PAYMENT");
  if (txn?.status === "COMPLETED" && step !== "RECEIPT") setStep("RECEIPT");
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full p-6 lg:p-12 space-y-8">
        ...
        {step === "SEARCH" && (
          <div className="space-y-12 py-20 text-center animate-in fade-in">
             <div className="space-y-4">
                <h2 className="text-4xl font-display font-black uppercase tracking-tighter">Merchant Gateway</h2>
                <p className="text-slate-500 text-sm max-w-sm mx-auto">Enter the unique Merchant ID provided to you by the seller to browse their catalog and request quotes.</p>
             </div>
             <div className="flex flex-col items-center gap-4 max-w-md mx-auto">
                <Input 
                  placeholder="Enter Store ID (e.g. USER-UID)" 
                  value={storeIdInput} 
                  onChange={e => setStoreIdInput(e.target.value)}
                  className="bg-white/[0.03] border-white/10 h-14 rounded-2xl text-center font-mono tracking-widest uppercase"
                />
                <Button onClick={handleSearch} className="w-full h-14 rounded-2xl bg-primary text-white font-bold group">
                  Enter Storefront <ArrowRight className="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform" />
                </Button>
             </div>
        ...
        {step === "STOREFRONT" && seller && (
          <div className="space-y-8 animate-in slide-in-from-bottom-4">
             <div className="space-y-1">
                ...
                <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Store Catalog</p>
             </div>
             ...
             {selectedProduct && (
               <div className="bg-white/[0.02] border border-white/10 p-8 rounded-[2rem] space-y-6">
                  <h3 className="text-sm font-bold uppercase tracking-widest text-slate-500">Requesting: {selectedProduct.name}</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                       <Label className="text-[10px] uppercase font-bold text-slate-500">Your Name</Label>
                    ...
                    <div className="space-y-2">
                       ...
                       <Input type="number" value={isNaN(buyerForm.quantity) ? "" : buyerForm.quantity} onChange={e => setBuyerForm(f => ({ ...f, quantity: parseInt(e.target.value) || 0 }))} className="bg-black/20 border-white/10" />
                    </div>
                  </div>
                  <div className="space-y-2">
                     <Label className="text-[10px] uppercase font-bold text-slate-500">Custom Message</Label>
                  ...
                  <Button onClick={handleRequestQuote} className="w-full h-14 bg-primary rounded-2xl font-bold">
        ...
        {step === "PENDING" && txn && (
          <div className="text-center py-20 space-y-8 animate-in zoom-in-95">
             ...
             <div className="space-y-2">
                <h2 className="text-3xl font-display font-black">Request Pending</h2>
                <p className="text-sm text-slate-500 max-w-sm mx-auto">The merchant is currently reviewing your request for <strong>{txn.product_name}</strong>. This usually takes a few minutes.</p>
             </div>
        ...
        {step === "PAYMENT" && txn && (
           <div className="space-y-8 animate-in slide-in-from-bottom-8">
              <div className="bg-primary/10 border border-primary/20 rounded-[3rem] p-12 text-center">
                 <p className="text-[10px] uppercase font-bold text-primary/60 mb-2">Quote Approved</p>
              ...
              <div className="bg-white/[0.03] border border-white/10 p-8 rounded-[2rem] space-y-4">
                 <h4 className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Bank Transfer Details</h4>
                 {[
                    ["Bank Name", txn.bank_details?.bank_name],
        ...
        {step === "RECEIPT" && txn && (
          <div className="text-center py-12 space-y-8 animate-in zoom-in-95">
             ...
             <h2 className="text-4xl font-display font-black">Payment Verified</h2>
             
             <div className="bg-white/[0.03] border-2 border-dashed border-white/10 rounded-[3rem] p-10 text-left space-y-6">
                <div className="flex justify-between items-start">
                   <div>
                      <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Receipt For</h4>
                ...
                <div className="grid grid-cols-2 gap-8 py-6 border-y border-white/5">
                   ...
                   <div>
                      <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Date</h4>
                      <p className="text-sm">{new Date().toLocaleDateString()}</p>
                   </div>
                </div>
                <div className="flex justify-between items-center">
                   <span className="text-xl font-display font-black uppercase">Total Paid</span>
             ...
             <Button variant="ghost" className="text-slate-500 hover:text-white" onClick={() => setStep("SEARCH")}>
                <RotateCcw className="w-4 h-4 mr-2" /> Start New Transaction
             </Button>
          </div>
        )}
      </main>
